...
void Graph::search() {
	...
	if(kplex.size() < UB) {
		...
		while(m && kplex.size()<UB) {
			ui idx, key; linear_heap2->get_min(idx, key);
			ui eu=edge_list[idx<<1], ev=edge_list[(idx<<1)+1];
			...
			if(kplex.size() != pre_size && kplex.size()> 2*K-2) {
				...
				m -= 2*peeling2(idx, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, true, kplex.size()+1-2*K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
			}
			else if(kplex.size()>2*K-2) m -= 2*peeling2(idx, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, false, kplex.size()+1-2*K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
			else m -= 2*peeling2(idx, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, false, 0, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
		}
