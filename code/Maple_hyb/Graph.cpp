...
void Graph::search() {
	...
	if(kplex.size() < UB) {
		...
		int max_n = n - Qv_n; Qv_n=0;
		ListLinearHeap *linear_heap2 = new ListLinearHeap(max_m>>1, n-2);
		linear_heap2->init2(max_m>>1, n-2, deleted, tri_cnt);
		...
		while(m && kplex.size()<UB) {
			ui idx, key; linear_heap->get_min(idx, key);
			ui idx2, key2; linear_heap2->get_min(idx2, key2);
			if(0.8*(key+K)<=key2+2*K){
				// if(key+K <= kplex.size()) {
				// 	Qv[0] = idx; Qv_n = 1;
				...
				ui *ids = Qv; ui ids_n = 0;
				ui pre_size; ui pre_size0 = kplex.size();
				do{
					pre_size=kplex.size();
					extract_subgraph_and_prune(idx, ids, ids_n, rid, vp, Qe, t_degree, exists, pend, deleted, edgelist_pointer);
					if(ids_n > kplex.size()&& vp.size()*2 < m) subgraph_heuri(ids, ids_n, vp, rid, Qv, Qe, exists);
				}
				...
				Qv[0] = idx; Qv_n = 1;
				if(kplex.size() != pre_size0 && kplex.size()> 2*K-2) {
					for(ui j = 0;j < kplex.size();j ++) kplex[j] = ids[kplex[j]];
					m -= 2*peeling3(-1, linear_heap, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, true, kplex.size()+1-2*K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
				}
				else if(kplex.size()>2*K-2) m -= 2*peeling3(-1, linear_heap, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, false, kplex.size()+1-2*K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
				else m -= 2*peeling3(-1, linear_heap, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, false, 0, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
			} else{
				ui eu=edge_list[idx2<<1], ev=edge_list[(idx2<<1)+1];
				ui *ids = Qv; ui ids_n = 0;
				ui pre_size; ui pre_size0 = kplex.size();
				do{
					pre_size=kplex.size();
					extract_subgraph_and_prune2(eu, ev, ids, ids_n, rid, vp, Qe, t_degree, exists, pend, deleted, edgelist_pointer);
					if(ids_n > kplex.size()&& vp.size()*2 < m) subgraph_heuri(ids, ids_n, vp, rid, Qv, Qe, exists);
				}
				while(kplex.size()!=pre_size);
				if(ids_n > kplex.size()) {
					kplex_solver->load(ids_n, vp);
					kplex_solver->run2(K, kplex);
				}
				Qv_n=0;
				if(kplex.size() != pre_size0 && kplex.size()> 2*K-2) {
					for(ui j = 0;j < kplex.size();j ++) kplex[j] = ids[kplex[j]];
					m -= 2*peeling3(idx2, linear_heap, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, true, kplex.size()+1-2*K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
				}
				else if(kplex.size()>2*K-2) m -= 2*peeling3(idx2, linear_heap, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, false, kplex.size()+1-2*K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
				else m -= 2*peeling3(idx2, linear_heap, linear_heap2, Qv, Qv_n, kplex.size()+1-K, Qe, false, 0, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
			}
		}

		printf("*** Search time: %s \n", Utility::integer_to_string(tt.elapsed()).c_str());

		if(kplex.size() > old_size) for(ui i = 0;i < kplex.size();i ++) kplex[i] = out_mapping[kplex[i]];
...
void Graph::subgraph_heuri(ui *ids, ui &_n, vector<pair<int,int> > &edge_list, ui *rid, ui *Qv, ui *Qe, char *exists) {
	ui s_n; ept s_m;
...
void Graph::extract_subgraph_and_prune2(ui eu, ui ev, ui *ids, ui &ids_n, ui *rid, vector<pair<int,int> > &vp, ui *Q, ui* degree, char *exists, ept *pend, char *deleted, ui *edgelist_pointer) {
	vp.clear();
	ids_n = 0; ids[ids_n++] = eu; exists[eu] = 1; ids[ids_n++] = ev; exists[ev] = 1;
	ui u_n = pstart[eu];
	for(ept i = pstart[eu];i < pend[eu];i ++) if(!deleted[edgelist_pointer[i]]) {
		edges[u_n] = edges[i]; edgelist_pointer[u_n++] = edgelist_pointer[i];
		ui w = edges[i]; if(w==ev) continue;
		exists[w] = 3;
	}
	pend[eu] = u_n;
	ui v_n = pstart[ev];
	for(ept i = pstart[ev];i < pend[ev];i ++) if(!deleted[edgelist_pointer[i]]) {
		edges[v_n] = edges[i]; edgelist_pointer[v_n++] = edgelist_pointer[i];
		ui w = edges[i]; if(w==eu) continue;
		if(exists[w]==3) ids[ids_n++] = w, exists[w] = 2;
		else exists[w] = 4;
	}
	pend[ev] = v_n;
	ui u; ui Q_n = 0;
	for(ui i = 2;i < ids_n;i ++) {
		u = ids[i];
		u_n = pstart[u];
		degree[u] = 0;
		for(ept j = pstart[u];j < pend[u];j ++) if(!deleted[edgelist_pointer[j]]) {
		...
		pend[u] = u_n;
		if(degree[u]+3*K <= kplex.size()) Q[Q_n++] = u;
	}
	for(ui i = 0;i < Q_n;i ++) {
		u = Q[i]; exists[u] = 10;
		for(ept j = pstart[u];j < pend[u];j ++) if(exists[edges[j]] == 2) {
			if( (degree[edges[j]]--) + 3*K == kplex.size()+1) {
				assert(Q_n < m/2);
				Q[Q_n++] = edges[j];
			}
		}
	...
	int cursor=2;
	for(ui i = 2; i < ids_n;i ++) if(exists[ids[i]]!=10) ids[cursor++] = ids[i];
	ids_n=cursor;
	
	if(ids_n + 2*K-2 <= kplex.size()) {
	...
	ui nr_size = ids_n;
	for(ept i = pstart[eu];i < pend[eu]; i++) {
		ui w = edges[i]; if(exists[w]==3) ids[ids_n++]=w;
	}
	Q_n = 0;
	for(ui i = nr_size;i < ids_n;i ++) {
		u = ids[i];
		...
		for(ept j = pstart[u];j < pend[u];j ++) if(!deleted[edgelist_pointer[j]]) {
			edges[u_n] = edges[j]; edgelist_pointer[u_n++] = edgelist_pointer[j];
			if(exists[edges[j]] == 2) ++ degree[u];
			if(exists[edges[j]] == 3) ++ cnt3;
		}
		pend[u] = u_n;
		if(degree[u]+3*K <= kplex.size()+2) Q[Q_n++] = u;
		else if(degree[u]+cnt3+2*K <= kplex.size()) Q[Q_n++] = u;
		degree[u]+=cnt3;
	}
	for(ui i = 0;i < Q_n;i ++) {
		u = Q[i]; if(exists[u]==10) continue;
		exists[u] = 10;
		for(ept j = pstart[u];j < pend[u];j ++) if(exists[edges[j]] == 3) {
			if( (degree[edges[j]]--) + 2*K == kplex.size()+1) {
				assert(Q_n < m/2);
				Q[Q_n++] = edges[j];
			}
	...
	cursor=nr_size;
	for(ui i = nr_size; i < ids_n;i ++) if(exists[ids[i]]!=10) ids[cursor++] = ids[i];
	ids_n=cursor;

	ui u_size=ids_n;
	for(ept i = pstart[ev];i < pend[ev];i ++) {
		ui w = edges[i]; if(exists[w]==4) ids[ids_n++]=w;
	}
	Q_n = 0;
	for(ui i = u_size;i < ids_n;i ++) {
		u = ids[i];
		u_n = pstart[u];
		degree[u] = 0;
		int cnt3=1, cnt4=0;
		for(ept j = pstart[u];j < pend[u];j ++) if(!deleted[edgelist_pointer[j]]) {
			...
			if(exists[edges[j]] == 2) ++ degree[u];
			if(exists[edges[j]] == 3) ++ cnt3;
			if(exists[edges[j]] == 4) ++ cnt4;
		}
		pend[u] = u_n;
		if(degree[u]+3*K <= kplex.size()+2) Q[Q_n++] = u;
		else if(degree[u]+cnt3+2*K <= kplex.size()+2) Q[Q_n++] = u;
		else if(degree[u]+cnt4+2*K <= kplex.size()) Q[Q_n++] = u;
		degree[u]+=cnt4;
	}
	for(ui i = 0;i < Q_n; i++) {
		u = Q[i]; if(exists[u]==10) continue;
		exists[u] = 10;
		for(ept j = pstart[u];j < pend[u];j ++) if(exists[edges[j]] == 4) {
			if( (degree[edges[j]]--) + 2*K == kplex.size()+1) {
				assert(Q_n < m/2);
				Q[Q_n++] = edges[j];
			}
	...
	cursor=u_size;
	for(ui i = u_size; i < ids_n;i ++) if(exists[ids[i]]!=10) ids[cursor++] = ids[i];
	ids_n=cursor;

	ui v_size=ids_n;
	for(ui i=2;i<nr_size;++i){
		u = ids[i];
		for(ept j = pstart[u];j < pend[u];j ++) {
			if(!exists[edges[j]]) ids[ids_n++]=edges[j], exists[edges[j]]=5;
		}
	}
	
	for(ui i=v_size;i<ids_n;++i){
		u = ids[i];
		u_n = pstart[u];
		degree[u] = 0;
		int cnt3=0, cnt4=0;
		...
		pend[u] = u_n;
		if(degree[u]+3*K <= kplex.size()+4) exists[u] = 10;
		else if(degree[u]+cnt3+2*K <= kplex.size()+2) exists[u] = 10;
		else if(degree[u]+cnt4+2*K <= kplex.size()+2) exists[u] = 10;
	}
	cursor=v_size;
	for(ui i = v_size; i < ids_n;i ++) if(exists[ids[i]]!=10) ids[cursor++] = ids[i];
	ids_n=cursor;
	...
	for(ui i=0;i<ids_n;++i){
		u = ids[i];
		degree[u] = 0;
		for(ept j = pstart[u];j < pend[u];j ++) if(exists[edges[j]]>0 && exists[edges[j]]<=5) ++ degree[u];
		if(degree[u]+K <= kplex.size()) Q[Q_n++]=u;
	}
	for(ui i = 0;i < Q_n; i++) {
		u = Q[i]; exists[u] = 10;
		for(ept j = pstart[u];j < pend[u];j ++) if(exists[edges[j]]>0 && exists[edges[j]]<=5) {
			if( (degree[edges[j]]--) + K == kplex.size()+1) {
				assert(Q_n < m/2);
				Q[Q_n++] = edges[j];
			}
		}
	...
	cursor=0;
	for(ui i = 0; i < ids_n;i ++) if(exists[ids[i]]!=10) ids[cursor++] = ids[i];
	ids_n=cursor;
	...
	for(ui i = 0;i < ids_n;i ++) {
		u = ids[i];
		for(ept j = pstart[u];j < pend[u];j ++) {
			if(edges[j] > u && exists[edges[j]]>0 && exists[edges[j]]<=5) vp.push_back(make_pair(rid[u], rid[edges[j]]));
		}
	}
	for(ui i = 0;i < n;i ++) exists[i] = 0;
}
...
ept Graph::peeling2(ui critical_edge, ListLinearHeap *linear_heap2, ui *Qv, ui &Qv_n, ui d_threshold, ui *Qe, bool initialize_Qe, ui t_threshold, ui *tri_cnt, ui *active_edgelist, ui &active_edgelist_n, ui *edge_list, ui *edgelist_pointer, char *deleted, ui *degree, ept *pstart, ept *pend, ui *edges, char *exists) {
	ept Qe_n = 0; Qe[Qe_n++]=critical_edge;
#ifndef NO_TRUSS_PRUNE
	if(initialize_Qe) {
		ept active_edgelist_newn = 0;
		for(ept j = 0;j < active_edgelist_n;j ++) if(!deleted[active_edgelist[j]]) {
			if(active_edgelist[j]==critical_edge) continue;
			if(linear_heap2->get_key(active_edgelist[j]) < t_threshold) Qe[Qe_n++] = active_edgelist[j];
			else active_edgelist[active_edgelist_newn ++] = active_edgelist[j];
		}
		active_edgelist_n = active_edgelist_newn;
	}
	...
	while(Qv_idx < Qv_n || Qe_n) {
		if(Qe_n == 0) {
			...
			for(ept k = pstart[u];k < pend[u];k ++){
				...
				linear_heap2->del(edgelist_pointer[k]);
			}
			deleted_edges_n += pend[u] - pstart[u];
			degree[u] = 0;
			//printf("Removed %u\n", u);
			...
			for(ept k= pstart[u];k < pend[u];k ++) {
				...
				for(ept x = pstart[v];x < pend[v];x ++) if(!deleted[edgelist_pointer[x]]) {
					edges[v_n] = edges[x]; edgelist_pointer[v_n++] = edgelist_pointer[x];
					if(edges[x] > v && exists[edges[x]]) {
						if( linear_heap2->decrement(edgelist_pointer[x],1) == t_threshold) Qe[Qe_n++] = edgelist_pointer[x];
					}
				}
				pend[v] = v_n;
				assert(degree[v]);
		...
		for(ept j = 0;j < Qe_n;j ++) {
			ept idx = Qe[j];
			ui u = edge_list[idx<<1], v = edge_list[(idx<<1)+1];
			ui tri_n = linear_heap2->get_key(idx);
			//printf("remove %u %u\n", u, v);
			deleted[idx] = 1; linear_heap2->del(idx);
			...
			//if(false) {
				...
				for(ept k = pstart[v];k < pend[v];k ++) if(!deleted[edgelist_pointer[k]]) {
					...
					if(tri_n&&find(u, edges[k], start, pend[u], deleted, idx, edgelist_pointer, edges)) {
						-- tri_n;
						if( linear_heap2->decrement(idx,1) == t_threshold) Qe[Qe_n++] = idx;
						if( linear_heap2->decrement(edgelist_pointer[k],1) == t_threshold) Qe[Qe_n++] = edgelist_pointer[k];
						assert(!deleted[edgelist_pointer[k]]);
						assert(!deleted[idx]);
					}
				}
				pend[v] = v_n;
				...
				while(true) {
					...
					if(edges[ii] == edges[jj]) {
						...
						if( linear_heap2->decrement(edgelist_pointer[ii],1) == t_threshold) Qe[Qe_n++] = edgelist_pointer[ii];
						if( linear_heap2->decrement(edgelist_pointer[jj],1) == t_threshold) Qe[Qe_n++] = edgelist_pointer[jj];
